...
// This also works on `radio` and `checkbox`, resulting validation of multi times. Filter them out?			
			self.form.addEventListener('focusout', (event) => {
				if (event.target.classList.contains(self.settings.inputClass)) {
...
// Forbid any whitespace
	...
	sanitiseForm() {
		const inputs = this.form.querySelectorAll(`.${this.settings.inputClass}`);
